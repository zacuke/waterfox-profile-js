 ...
 */
function WaterfoxProfile(options) {
...
WaterfoxProfile.prototype._copy = function (profileDirectory, cb) {
  fs.copy(
    profileDirectory,
    this.profileDir,
    {
...
/**
 * creates a profile Profile from an existing waterfox profile directory asynchronously
 *
...
WaterfoxProfile.copy = function (options, cb) {
  var opts = parseOptions(options);
  if (!opts.profileDirectory) {
    cb &&
      cb(new Error('waterfoxProfile: .copy() requires profileDirectory option'));
    return;
  }
  var profile = new WaterfoxProfile({
 ...
 */
WaterfoxProfile.copyFromUserProfile = function (opts, cb) {
  if (!opts.name) {
    cb &&
      cb(
        new Error(
          'waterfoxProfile: .copyFromUserProfile() requires a name options'
        )
      );
    return;
  }
 ...
 */
WaterfoxProfile.prototype.deleteDir = function (cb) {
  var self = this;
 ...
 *
 */
WaterfoxProfile.prototype._cleanOnExit = function () {
  if (fs.existsSync(this.profileDir)) {
    try {
      // node 14.4+
      if (fs.rmSync) {
      ...
      console.warn(
        '[waterfox-profile] cannot delete profileDir on exit',
        this.profileDir,
        e
      );
    }
 ...
 * @param {boolean} true
 */
WaterfoxProfile.prototype.shouldDeleteOnExit = function (bool) {
 ...
 */
WaterfoxProfile.prototype.willDeleteOnExit = function () {
 ...
 */
WaterfoxProfile.prototype.setPreference = function (key, value) {
  var cleanValue = '';
  if (value === true) {
    cleanValue = 'true';
  } else if (value === false) {
 ...
 */
WaterfoxProfile.prototype.addExtension = function (extension, cb) {
...
WaterfoxProfile.prototype.addExtensions = function (extensions, cb) {
 ...
 *
 */
WaterfoxProfile.prototype.updatePreferences = function () {
 ...
 *
 */
WaterfoxProfile.prototype.path = function () {
 ...
 *
 */
WaterfoxProfile.prototype.canAcceptUntrustedCerts = function () {
 ...
 *
 */
WaterfoxProfile.prototype.setAcceptUntrustedCerts = function (val) {
 ...
 *
 */
WaterfoxProfile.prototype.canAssumeUntrustedCertIssuer = function () {
  return this._sanitizePref(
 ...
 *
 */
WaterfoxProfile.prototype.setAssumeUntrustedCertIssuer = function (val) {
 ...
 *
 */
WaterfoxProfile.prototype.nativeEventsEnabled = function () {
  return this._sanitizePref(
 ...
 *
 */
WaterfoxProfile.prototype.setNativeEventsEnabled = function (val) {
 ...
 */
WaterfoxProfile.prototype.encoded = function (cb) {
  try {
    var self = this;
...
WaterfoxProfile.prototype.encode = WaterfoxProfile.prototype.encoded;
 ...
 */
WaterfoxProfile.prototype.setProxy = function (proxy) {
  if (!proxy || !proxy.proxyType) {
    throw new Error('waterfoxProfile: not a valid proxy type');
  }
...
// private
WaterfoxProfile.prototype._writeUserPrefs = function (userPrefs) {
  var content = '';
...
WaterfoxProfile.prototype._readExistingUserjs = function () {
  var self = this,
    regExp = /user_pref\(['"](.*)["'],\s*(.*)\)/,
...
WaterfoxProfile.prototype._installExtension = function (addon, cb) {
  ...
  this._addonDetails(addon, function (addonDetails) {
    ...
    if (!addonId) {
      cb(new Error('WaterfoxProfile: the addon id could not be found!'));
    }
...
WaterfoxProfile.prototype._addonDetails = function (addonPath, cb) {
  var details = {
      id: null,
      name: null,
      unpack: true,
...
WaterfoxProfile.prototype._createTempFolder = function (prefix) {
  // Don't use an empty string for `prefix` or it might not work on some CIs (Travis)
  return (
    fs.mkdtempSync(path.resolve(os.tmpdir(), prefix || 'waterfox-profile')) + '/'
  );
};

WaterfoxProfile.prototype._sanitizePref = function (val) {
  if (val === 'true') {
    return true;
  }
  if (val === 'false') {
...
WaterfoxProfile.prototype._setManualProxyPreference = function (key, setting) {
  if (!setting || setting === '') {
    return;
  }
...
WaterfoxProfile.Finder = Finder;

module.exports = WaterfoxProfile;
