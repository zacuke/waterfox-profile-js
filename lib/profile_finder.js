...
function locateLinuxDirectory() {
  ...
  var homes = [
    ...
    path.join(userHomeDir, 'snap/waterfox/common', '.mozilla/waterfox'),
    path.join(userHomeDir, 'snap/waterfox/common', '.config/mozilla/waterfox'),
    path.join(userHomeDir, '.var/app/org.mozilla.waterfox', '.mozilla/waterfox'),
    path.join(
      userHomeDir,
      '.var/app/org.mozilla.waterfox',
      '.config/mozilla/waterfox'
    ),
  ];
  ...
  const defaultLocation = path.join(userHomeDir, '.mozilla/waterfox');
  ...
  if (!fs.existsSync(configDir)) {
    console.warn('detected waterfox profile directory:', configDir, 'not found');
  }
...
function locateUserDirectory(platform) {
  ...
  switch (platform) {
    ...
    case 'win32':
      return path.join(process.env.APPDATA, '/Mozilla/Waterfox');
    default:
