...
var minimist = require('minimist'),
  util = require('util'),
  fs = require('fs'),
  WaterfoxProfile = require('./waterfox_profile');
...
function displayHelp() {
  console.log('usage: waterfox-profile [-v] [-p profilename || -c profile_dir] [-e extension_path1 [-e extension_path2]] [-o destination_dir || --ouput destination_dir || -b [file_path] || --base64 [file_path]]');
  console.log('');
  ...
  console.log('-b [file_path](or --base64): write the encoded waterfox profile to stdout or to file_path if specified.');
}
...
function createProfile(cb) {
  if (profileName || profileDirectory) {
    ...
    log(util.format('calling WaterfoxProfile.`%s` method with options %s', copyMethod, JSON.stringify(copyOptions)));
    WaterfoxProfile[copyMethod](copyOptions, function(err, fp) {
      cb(err, fp);
    });
    return;
